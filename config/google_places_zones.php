<?php

/**
 * Zones d’import Google Places — couverture du Sénégal (14 régions + villes clés).
 * Rayon max par zone : 50 km (limite API Google Nearby Search).
 * Les doublons sont fusionnés via google_place_id.
 */
return [
    ['id' => 'dakar', 'name' => 'Dakar', 'lat' => 14.7167, 'lng' => -17.4677],
    ['id' => 'pikine', 'name' => 'Pikine / Guédiawaye', 'lat' => 14.7645, 'lng' => -17.3907],
    ['id' => 'rufisque', 'name' => 'Rufisque / Bargny', 'lat' => 14.7162, 'lng' => -17.2736],
    ['id' => 'thies', 'name' => 'Thiès', 'lat' => 14.7886, 'lng' => -16.9260],
    ['id' => 'mbour', 'name' => 'Mbour / Saly', 'lat' => 14.4167, 'lng' => -16.9667],
    ['id' => 'saint_louis', 'name' => 'Saint-Louis', 'lat' => 16.0179, 'lng' => -16.4896],
    ['id' => 'louga', 'name' => 'Louga', 'lat' => 15.6167, 'lng' => -16.2167],
    ['id' => 'diourbel', 'name' => 'Diourbel', 'lat' => 14.6550, 'lng' => -16.2314],
    ['id' => 'touba', 'name' => 'Touba / Bambey', 'lat' => 14.8667, 'lng' => -15.8833],
    ['id' => 'kaolack', 'name' => 'Kaolack', 'lat' => 14.1519, 'lng' => -16.0726],
    ['id' => 'fatick', 'name' => 'Fatick', 'lat' => 14.3390, 'lng' => -16.4111],
    ['id' => 'foundiougne', 'name' => 'Foundiougne', 'lat' => 14.1333, 'lng' => -16.4667],
    ['id' => 'kaffrine', 'name' => 'Kaffrine', 'lat' => 14.1056, 'lng' => -15.5417],
    ['id' => 'koungheul', 'name' => 'Koungheul', 'lat' => 13.9833, 'lng' => -14.8000],
    ['id' => 'tambacounda', 'name' => 'Tambacounda', 'lat' => 13.7707, 'lng' => -13.6673],
    ['id' => 'bakel', 'name' => 'Bakel', 'lat' => 14.9050, 'lng' => -12.4594],
    ['id' => 'kedougou', 'name' => 'Kédougou', 'lat' => 12.5605, 'lng' => -12.1747],
    ['id' => 'kolda', 'name' => 'Kolda', 'lat' => 12.8837, 'lng' => -14.9500],
    ['id' => 'velingara', 'name' => 'Vélingara', 'lat' => 13.1500, 'lng' => -14.1167],
    ['id' => 'sedhiou', 'name' => 'Sédhiou', 'lat' => 12.7081, 'lng' => -15.5569],
    ['id' => 'ziguinchor', 'name' => 'Ziguinchor', 'lat' => 12.5681, 'lng' => -16.2733],
    ['id' => 'matam', 'name' => 'Matam', 'lat' => 15.6559, 'lng' => -13.2554],
    ['id' => 'podor', 'name' => 'Podor', 'lat' => 16.7167, 'lng' => -14.9667],
    ['id' => 'linguere', 'name' => 'Linguère', 'lat' => 15.3833, 'lng' => -15.1167],
];
